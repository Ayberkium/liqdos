...
int siringa_alt_switch = digitalRead(E_MIN_PIN); 
int siringa_ust_switch = digitalRead(E_MAX_PIN); 
int x = 0;
  ...
  if ((digitalRead(eight_KG_MAGNETS) == HIGH) && (digitalRead(thee_KG_MAGNET) == HIGH) && digitalRead(E0_DIR_PIN == HIGH) && (siringa_alt_switch == LOW))
{
...
digitalWrite(E0_DIR_PIN, LOW); //Changes the direction of rotation
...
digitalWrite(E0_DIR_PIN, HIGH); //Changes the direction of rotation
  for(int s = 0; s < 100; s++) {
     delay(1);
    digitalWrite(E0_STEP_PIN, HIGH);
    delayMicroseconds(800);
